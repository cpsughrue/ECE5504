...
class SinExampleModuleImp(outer: SinExample)(implicit p: Parameters) extends LazyRoCCModuleImp(outer) with HasCoreParameters {
  ...
  val lookupSinModule = Module(new LookupSinModule)
  lookupSinModule.io.in := 0.S
  val chebyshevSinModule = Module(new ChebyshevSinModule)
  chebyshevSinModule.io.in := 0.S

  when (doRead) {
    printf("Received a calculate_sin (lookup) instruction with binary value: %b\n", cmd.bits.rs2);

    // Pass the input value to the LookupSinModule
    lookupSinModule.io.in := value.asSInt

    // Debug output for the sine calculation result
    printf("Sine result from LookupSinModule: %d\n", lookupSinModule.io.out)
  }

  when (doLoad) {
    printf("Received a calculate_sin (chebyshev) instruction with binary value: %b\n", cmd.bits.rs2);

    // Pass the input value to the ChebyshevSinModule
    chebyshevSinModule.io.in := value.asSInt

    // Debug output for the sine calculation result
    printf("Sine result from ChebyshevSinModule: %d\n", chebyshevSinModule.io.out)
  }
  ...
  when (doRead) {
    io.resp.bits.data := lookupSinModule.io.out.asUInt
  } .otherwise {
    io.resp.bits.data := chebyshevSinModule.io.out.asUInt
  }
